...
use std::borrow::Cow;
use farmfe_core::{
  swc_common::{comments::Comments, errors::HANDLER, EqIgnoreSpan, Span, Spanned, DUMMY_SP},
  swc_ecma_ast::*,
};
use farmfe_toolkit::{
  swc_atoms::{js_word, JsWord},
  swc_ecma_utils::{quote_ident, quote_str},
};
              ...
              match &**prop {
                Prop::Shorthand(ident) => Some((
                  ...
                  Expr::Arrow(ArrowExpr {
                    params: vec![],
                    body: Box::new(BlockStmtOrExpr::Expr(Box::new(Expr::Ident(ident.clone())))),
                    is_async: false,
                    is_generator: false,
                    span: DUMMY_SP,
                    type_params: None,
                ...
                Prop::KeyValue(KeyValueProp { key, value }) => {
                  ...
                  Some((
                    ...
                    if value.is_lit() {
                      ...
                      Expr::Arrow(ArrowExpr {
                        params: vec![],
                        body: Box::new(BlockStmtOrExpr::Expr(value.clone())),
                        is_async: false,
                        is_generator: false,
                        span: DUMMY_SP,
                        type_params: None,
    ...
    Some(match defaults {
      ...
      Some(Defaults::Dynamic(expr)) => {
        ...
        Expr::Call(CallExpr {
          ...
          args: vec![
            ExprOrSpread {
              expr: Box::new(Expr::Object(self.build_props_type(first_param_type, None))),
              spread: None,
            },
            ExprOrSpread {
          ...
          match &*ident.sym {
            ...
            "Required" => {
                ...
                .and_then(|params| params.params.first())
              {
                ...
                props.extend(inner_props.into_iter().map(|mut prop| {
                  match &mut prop {
                    RefinedTsTypeElement::Property(TsPropertySignature { optional, .. })
                    | RefinedTsTypeElement::MethodSignature(TsMethodSignature {
                      optional, ..
                    }) => {
                      *optional = false;
            ...
            "Pick" => {
                ...
                .and_then(|params| params.params.first().zip(params.params.get(1)))
              {
                ...
                props.extend(inner_props.into_iter().filter(|prop| match prop {
                  RefinedTsTypeElement::Property(TsPropertySignature { key, .. })
                  | RefinedTsTypeElement::MethodSignature(TsMethodSignature { key, .. })
                  | RefinedTsTypeElement::GetterSignature(TsGetterSignature { key, .. }) => {
                    match &**key {
            ...
            HANDLER.with(|handler| {
              handler.span_err(ident.span, "Types from other modules can't be resolved.");
            });
        }
      }
  ...
  fn resolve_string_or_union_strings(&self, ty: &TsType) -> Vec<JsWord> {
    match ty {
      ...
      TsType::TsTypeRef(TsTypeRef {
        ...
        if let Some(aliased) = self.type_aliases.get(&(ident.sym.clone(), ident.span.ctxt)) {
      ...
      _ => {
        HANDLER.with(|handler| handler.span_err(ty.span(), "Unsupported type as index key."));
        vec![]
      }
    }
  }
          ...
          let mut properties = match index {
            TsType::TsKeywordType(TsKeywordType {
              ...
              .filter_map(|element| match element {
                ...
                TsTypeElement::TsIndexSignature(TsIndexSignature { type_ann, .. }) => {
                  type_ann.as_ref().map(|type_ann| type_ann.type_ann.clone())
                }
            ...
            | TsType::TsTypeRef(..) => {
              ...
              interface
                ...
                .filter_map(|element| match element {
                  ...
                  TsTypeElement::TsMethodSignature(TsMethodSignature { key, .. }) => {
                    if let Expr::Ident(Ident { sym: key, .. })
                    | Expr::Lit(Lit::Str(Str { value: key, .. })) = &**key
                    {
                      if keys.contains(key) {
          ...
          Some(ArrayLit {
            elems: emits
              ...
              .flat_map(|emit| match emit {
                RefinedTsTypeElement::MethodSignature(TsMethodSignature { key, .. })
                | RefinedTsTypeElement::Property(TsPropertySignature { key, .. }) => match &*key {
                  Expr::Ident(ident) => vec![ident.sym.clone()],
                  Expr::Lit(Lit::Str(str)) => vec![str.value.clone()],
                  _ => vec![],
                },
                RefinedTsTypeElement::CallSignature(TsCallSignatureDecl { params, .. }) => params
                  .first()
                  ...
                  .map(|type_ann| self.resolve_string_or_union_strings(&type_ann.type_ann))
                  .unwrap_or_default(),
