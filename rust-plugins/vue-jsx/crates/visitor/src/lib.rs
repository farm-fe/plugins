  ...
  C: Comments,
{
  ...
  fn transform_jsx_element(&mut self, jsx_element: &JSXElement) -> Expr {
    ...
    if self.options.optimize {
      ...
      match dynamic_props {
        Some(dynamic_props) if !dynamic_props.is_empty() => vnode_call_args.push(ExprOrSpread {
          spread: None,
              ...
              elems: directives
                ...
                .map(|directive| {
                  let mut elems = vec![
                    Some(ExprOrSpread {
                      spread: None,
                      expr: Box::new(self.resolve_directive(&directive.name, jsx_element)),
                    }),
                    Some(ExprOrSpread {
                      spread: None,
                  ...
                  props.push(PropOrSpread::Prop(Box::new(Prop::KeyValue(KeyValueProp {
                    key: match &directive.argument {
                      ...
                      Some(Expr::Lit(Lit::Str(Str { value, .. }))) => {
                        dynamic_props.insert(Cow::from(value.to_string()));
                  ...
                  if let Some(modifiers) = directive.modifiers {
                    props.push(PropOrSpread::Prop(Box::new(Prop::KeyValue(KeyValueProp {
                      key: match &directive.argument {
                        ...
                        Some(Expr::Lit(Lit::Str(Str { value, .. }))) => {
                          PropName::Str(quote_str!(format!("{value}Modifiers")))
                        }
                        Some(expr) => PropName::Computed(ComputedPropName {
                          span: DUMMY_SP,
                          expr: Box::new(Expr::Bin(BinExpr {
                            ...
                            right: Box::new(Expr::Lit(Lit::Str(quote_str!("Modifiers")))),
                          })),
                        }),
                      },
                ...
                props.push(PropOrSpread::Prop(Box::new(Prop::KeyValue(KeyValueProp {
                    ...
                    Some(expr) => {
                      ...
                      PropName::Computed(ComputedPropName {
                        ...
                        expr: Box::new(Expr::Bin(BinExpr {
                          span: DUMMY_SP,
                          op: op!(bin, "+"),
                          left: Box::new(Expr::Lit(Lit::Str(quote_str!("onUpdate")))),
                          right: Box::new(expr),
                        })),
                      })
                    }
                  ...
                  value: Box::new(Expr::Arrow(ArrowExpr {
                    ...
                    body: Box::new(BlockStmtOrExpr::Expr(Box::new(Expr::Assign(AssignExpr {
                      ...
                      right: Box::new(Expr::Ident(quote_ident!("$event").into())),
                    })))),
                    is_async: false,
                    is_generator: false,
                    type_params: None,
          ...
          JSXAttrOrSpread::JSXAttr(jsx_attr) => {
            ...
            let attr_value = jsx_attr
              ...
              .map(|value| match value {
                JSXAttrValue::Lit(Lit::Str(str)) => Box::new(Expr::Lit(Lit::Str(quote_str!(
                  util::transform_text(&str.value)
                )))),
                JSXAttrValue::Lit(..) => {
                ...
                JSXAttrValue::JSXElement(element) => Box::new(Expr::JSXElement(element.clone())),
            ...
            if self.options.transform_on && (attr_name == "on" || attr_name == "nativeOn") {
    ...
    let elems = children
      ...
      .filter_map(|child| match child {
        ...
        JSXElementChild::JSXExprContainer(JSXExprContainer {
          ...
          if self.options.optimize {
            match &**expr {
              Expr::Ident(ident) if !ident.to_id().1.has_mark(self.unresolved_mark) => {
          ...
          if self.options.enable_object_slots {
            Expr::Cond(CondExpr {
              ...
              test: Box::new(Expr::Call(CallExpr {
                span: DUMMY_SP,
                callee: Callee::Expr(Box::new(Expr::Ident(self.generate_slot_helper()))),
                args: vec![ExprOrSpread {
                  spread: None,
                  expr: Box::new(expr.clone()),
                }],
  ...
  fn resolve_directive(&mut self, directive_name: &str, jsx_element: &JSXElement) -> Expr {
    match directive_name {
      ...
      "model" => match &jsx_element.opening.name {
        ...
        _ => {
          let typ =
            jsx_element.opening.attrs.iter().find_map(
              |jsx_attr_or_spread| match jsx_attr_or_spread {
  ...
  C: Comments,
{
  ...
  fn visit_mut_jsx_opening_element(&mut self, jsx_opening_element: &mut JSXOpeningElement) {
    ...
    let JSXAttrOrSpread::JSXAttr(JSXAttr { value, .. }) = jsx_opening_element.attrs.remove(index)
    else {
      unreachable!()
    };
